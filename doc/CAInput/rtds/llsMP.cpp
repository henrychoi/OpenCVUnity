...
static void llsMP_dispose(struct llsMP* me) {
  me->_available = me->_capacity = 0;
}
static unsigned char llsMP_init(struct llsMP* me, size_t capacity, size_t memsize) {
  size_t j = capacity - 1;
  ...
  do {
    me->_book[j] = (void*)((size_t)me->_pool + j * memsize);//VC++ nonsense
  } while(j--);
...
unsigned char llsMP_alloc(struct llsMP* me,
...
#ifdef WIN32
  me->_pool = _aligned_malloc(alignment, memsize * capacity);
  if(!me->_pool) {
#else
  ...
  if (zero) memset(me->_pool, 0, memsize * capacity);
...
struct llsMP* llsMP_new(size_t capacity, size_t memsize, size_t alignment
  ...
  if(!(llsMP_alloc(me, capacity, memsize, alignment, zero))) {
    free(me);
    return NULL;
  }  
  if(!llsMP_init(me, capacity, memsize)) {
